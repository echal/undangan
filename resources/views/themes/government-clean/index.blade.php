...
<style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .header-title { font-weight: 700; font-size: 14px; color: var(--dark); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .header-badge {
      background: var(--primary-light); color: var(--primary);
      font-size: 11px; font-weight: 700;
      padding: 4px 12px; border-radius: 50px; white-space: nowrap;
      border: 1px solid rgba(20,184,166,0.3); flex-shrink: 0;
    }
    ...
    .hero-banner-bg { position:absolute; inset:0; background-size:cover; background-position:center; opacity:0.15; }
    .hero::before { content:''; position:absolute; top:-60px; right:-60px; width:280px; height:280px; border-radius:50%; background:var(--primary-mid); opacity:0.10; }
    .hero::after  { content:''; position:absolute; bottom:-80px; left:-40px; width:200px; height:200px; border-radius:50%; background:#fff; opacity:0.04; }
    .hero-accent-bar { position:absolute; left:0; top:0; bottom:0; width:5px; background:linear-gradient(to bottom, rgba(255,255,255,0.5), transparent); }
    ...
    .hero-title { font-size:clamp(1.9rem,5.5vw,3.2rem); font-weight:800; color:#fff; line-height:1.2; margin-bottom:14px; }
    .hero-subtitle { font-size:.95rem; color:rgba(255,255,255,0.65); line-height:1.7; max-width:520px; margin-bottom:32px; }
    .hero-meta { display:flex; flex-wrap:wrap; gap:18px; }
    .hero-meta-item { display:flex; align-items:center; gap:8px; color:rgba(255,255,255,0.82); font-size:14px; }
    .hero-meta-icon { width:32px; height:32px; background:rgba(255,255,255,0.12); border-radius:8px; display:flex; align-items:center; justify-content:center; font-size:15px; }

    /* ── FEATURES ROW ── */
    .features-row { background:var(--white); border-bottom:1px solid var(--border); padding:18px 24px; }
    ...
    .livestream-btn { display:inline-flex; align-items:center; gap:6px; padding:8px 18px; background:var(--primary); color:#fff; border-radius:50px; font-size:13px; font-weight:600; text-decoration:none; transition:background .2s; }
    .livestream-btn:hover { background:var(--primary-dark); }

    /* ── SECTION COMMON ── */
    section { padding:64px 24px; }
    .section-inner { max-width:800px; margin:0 auto; }
    .section-header { margin-bottom:36px; }
    .section-label { display:block; font-size:11px; font-weight:700; letter-spacing:3px; text-transform:uppercase; color:var(--primary); margin-bottom:8px; }
    .section-title { font-size:clamp(1.4rem,4vw,2rem); font-weight:700; color:var(--dark); line-height:1.3; }
    .section-divider { width:48px; height:3px; background:var(--primary); border-radius:2px; margin-top:12px; }
    ...
    .speaker-card { border:1px solid var(--border); border-radius:14px; overflow:hidden; background:var(--bg); transition:box-shadow .2s; }
    .speaker-card:hover { box-shadow:0 8px 24px rgba(15,118,110,0.10); }
    .speaker-photo-wrap { aspect-ratio:3/4; overflow:hidden; background:linear-gradient(135deg,#ccfbf1,#f0fdfa); }
    .speaker-photo-wrap img { width:100%; height:100%; object-fit:cover; }
    .speaker-photo-placeholder { width:100%; height:100%; display:flex; align-items:center; justify-content:center; font-size:4rem; color:#5eead4; }
    .speaker-body { padding:16px; }
    .speaker-role-badge { display:inline-block; background:var(--primary-light); color:var(--primary); font-size:10px; font-weight:700; letter-spacing:1.5px; text-transform:uppercase; padding:3px 10px; border-radius:50px; margin-bottom:8px; }
    ...
    .info-card { background:var(--white); border:1px solid var(--border); border-left:3px solid var(--primary); border-radius:14px; padding:20px; display:flex; align-items:flex-start; gap:14px; }
    .info-icon { width:40px; height:40px; min-width:40px; background:var(--primary-light); border-radius:10px; display:flex; align-items:center; justify-content:center; font-size:18px; }
    ...
    .gallery-item { aspect-ratio:1; border-radius:10px; overflow:hidden; cursor:pointer; box-shadow:0 2px 8px rgba(0,0,0,.06); transition:box-shadow .3s; }
    .gallery-item:hover { box-shadow:0 6px 20px rgba(15,118,110,0.18); }
    ...
    /* ── RSVP ── */
    .rsvp-section { background:var(--primary-light); }
    .rsvp-box { background:var(--white); border:1px solid rgba(20,184,166,0.3); border-radius:20px; padding:32px 28px; }
    .form-group { margin-bottom:18px; }
    .form-label { display:block; font-size:12px; font-weight:600; letter-spacing:.5px; color:var(--muted); margin-bottom:6px; }
    .form-input { width:100%; padding:11px 14px; border:1px solid var(--border); border-radius:10px; font-size:14px; font-family:'Inter',sans-serif; color:var(--dark); background:var(--bg); outline:none; transition:border-color .2s; }
    .form-input:focus { border-color:var(--primary); background:#fff; }
    .rsvp-radio-group { display:flex; gap:8px; flex-wrap:wrap; }
    .rsvp-radio-label { display:flex; align-items:center; gap:6px; padding:8px 14px; border:1px solid var(--border); border-radius:50px; cursor:pointer; font-size:13px; transition:all .2s; }
    .rsvp-radio-label:has(input:checked) { border-color:var(--primary); background:var(--primary-light); color:var(--primary); font-weight:600; }
    .rsvp-radio-label input { display:none; }
    .btn-primary { width:100%; padding:13px; background:var(--primary); color:#fff; border:none; border-radius:10px; font-size:14px; font-weight:600; font-family:'Inter',sans-serif; cursor:pointer; transition:background .2s; }
    ...
    .participant-avatar { width:40px; height:40px; min-width:40px; background:var(--primary); color:#fff; border-radius:50%; display:flex; align-items:center; justify-content:center; font-weight:700; font-size:15px; }
    ...
    .wish-card { background:var(--bg); border:1px solid var(--border); border-left:3px solid var(--primary); border-radius:12px; padding:16px 18px; }
    .wish-name { font-weight:700; font-size:13px; color:var(--primary); margin-bottom:5px; }
    .wish-message { font-size:14px; color:var(--muted); line-height:1.6; }

    /* ── FOOTER ── */
    .footer { background:var(--primary-dark); color:rgba(255,255,255,0.5); text-align:center; padding:28px 24px; font-size:12px; }
...
@php
  ...
  function speakerUrlGovt(string $path): string {
  ...
  <div class="section-inner">
    ...
    <div class="speakers-grid">
      @if ($hasSpeaker1)
        <div class="speaker-card">
          <div class="speaker-photo-wrap">
            @if ($sp1Photo) <img src="{{ speakerUrlGovt($sp1Photo) }}" alt="{{ $sp1Name }}" loading="lazy">
      ...
      @if ($hasSpeaker2)
        <div class="speaker-card">
          <div class="speaker-photo-wrap">
            @if ($sp2Photo) <img src="{{ speakerUrlGovt($sp2Photo) }}" alt="{{ $sp2Name }}" loading="lazy">
            @else <div class="speaker-photo-placeholder">👤</div> @endif
          </div>
          <div class="speaker-body">
      ...
      @if ($hasMc)
        <div class="speaker-card">
          <div class="speaker-photo-wrap">
            @if ($mcPhoto) <img src="{{ speakerUrlGovt($mcPhoto) }}" alt="{{ $mcName }}" loading="lazy">
...
<section class="info-section" id="detail">
  <div class="section-inner">
    <div class="section-header">
      <span class="section-label">Informasi Resmi</span>
          ...
          @if ($event->maps_link)
            <p class="info-value" style="margin-top:8px;">
              <a href="{{ $event->maps_link }}" target="_blank" rel="noopener"
                 style="display:inline-flex;align-items:center;gap:5px;background:var(--primary);color:#fff;padding:7px 14px;border-radius:8px;font-size:13px;text-decoration:none;font-weight:600;">
