<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            {{-- Statistik Pelaporan ASN (official_notice) --}}
            @if ($event->event_type === 'official_notice')
            @php
                $totalLaporan  = $event->noticeReports()->count();
                $totalTargetAsn = (int) $event->total_target_asn;
            @endphp
            <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6">
                <div class="flex items-center justify-between mb-4">
                    <div class="flex items-center gap-2">
                        ...
                        <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300">Statistik Pelaporan ASN</h4>
                    </div>
                    @if ($totalLaporan > 0)
                    <a href="{{ route('user.events.export-notices', $event) }}"
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    <div class="text-center p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
                        <p class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ $totalLaporan }}</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Sudah Melapor</p>
                    </div>
                    <div class="text-center p-3 bg-orange-50 dark:bg-orange-900/20 rounded-xl">
                        <p class="text-2xl font-bold text-orange-500 dark:text-orange-400">
                            {{ $totalTargetAsn > 0 ? max(0, $totalTargetAsn - $totalLaporan) : '—' }}
                        </p>
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Belum Melapor</p>
                    </div>
                    <div class="text-center p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                        <p class="text-2xl font-bold text-gray-700 dark:text-gray-200">
                            {{ $totalTargetAsn > 0 ? $totalTargetAsn : '—' }}
                        </p>
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Target ASN</p>
                    </div>
                </div>

                @if ($totalLaporan === 0)
                <p class="mt-3 text-xs text-gray-400 dark:text-gray-500 text-center">
                    Belum ada ASN yang melaporkan. Tombol download akan muncul setelah ada laporan.
                </p>
                @endif
            </div>
            @endif
